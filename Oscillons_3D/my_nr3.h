...
private:
	...
	int kk;
	T ***v;
	NRvector<T> r, theta, x, phi;
public:
	NRMat3d();
	NRMat3d(int n, int m, int k, NRvector<T> r_i);
	inline T** operator[](const int i);	//subscripting: pointer to row i
	inline const T* const * operator[](const int i) const;
	inline int dim1() const;
	inline int dim2() const;
	inline int dim3() const;
	inline T radius(int i) { return r[i] ; };
	~NRMat3d();
};
...
template <class T>
NRMat3d<T>::NRMat3d(int n, int m, int k, NRvector<T> r_i) : nn(n), mm(m), kk(k), v(new T**[n]), r(r_i)
{
	int i,j;
	v[0] = new T*[n*m];
	v[0][0] = new T[n*m*k];
